...
include_once('../manager/dataprovider/search_content.php');
...
if ($_GET['code'] == 'deleteitem') {
    if ($table && $id && $id_item) {
        ...
        if ($rs = mysql_fetch_array($db)) {
            ...
            $clsUrl->delete($table, $id, 0);
            $DB->query("DELETE FROM $table WHERE $id_item=$id");
            $search_obj->deleteSearch($id_item, $table, $id);
        }
    }
}
// delete module 
...
if ($_GET['code'] == 'loadcommbo') {
    ...
    $ct = new cat_tree(0);
    $ct->get_cat_tree(0, '');
    /* $ct->get_cat_string_admin($pid,$par_page,$category,$id_category,$data_type);      $navigator.=$catstring2; */
    $info['parentid1'] = $pid;
...
//    $tpl->assign("parentid", $info['parentid']);
    ...
    $image = clean_value($_GET['url']);
    $url = clean_url($_GET['url']) . ".html";
    $pid = intval($_GET['pid']);
    $sql = "INSERT INTO photo(image,id_category,url) VALUES('" . $image . "','" . $pid . "','" . $url . "')";
    $db = $DB->query($sql);
